...
const addRecipe = async (req, res) => {
    try {
        const { recipeName, ingredientsList, prepTime, difficultyLevel, imgUrls, description, steps, } = req.body;
        const user = req.user;
...
const addRecipeImageUrl = async (req, res) => {
    ...
    try {
        ...
        const updatedRecipe = await RecipeModel.findByIdAndUpdate({ _id: recipeId }, { imgUrls: updatedUrls }, { new: true });
        if (updatedRecipe) {
            return res.status(Constants.OK).json({
                isSuccess: true,
...
export { addRecipe, deleteRecipe, getRecipesByIngredients, addRecipeImageUrl, deleteOneImage, getAllRecipes, getOneRecipe, };
